...
Write-Host "`nUse the following Options:`n
            ...
            4 - Stop Service
            5 - Check CPU Usage
            6 - Check High Running Processess
            7 - Check Disk Space" -ForeGroundColor Yellow

$choice =Write-Host "`nChoice" -ForeGroundColor Yellow

$choice = Read-Host 
    If     ($choice -eq '1') {Get-Service -ComputerName $serverName
                              }
    ElseIf ($choice -eq '2') {Write-Host "ServiceName" -ForeGroundColor Yellow
                              $serviceName = Read-Host
                              Get-Service -ComputerName $serverName -Name $serviceName | Format-Table
                              }
    ElseIf ($choice -eq '3') {Write-Host "ServiceName" -ForeGroundColor Yellow
                              $serviceName = Read-Host
    ...
    ElseIf ($choice -eq '4') {Get-Service -ComputerName $serverName -Name $serviceName | Stop-Service -Force -Verbose
                              Write-Host "`nChecking the status of the service before finishing:`n"
                              Get-Service -ComputerName $serverName -Name $serviceName | Format-Table
                              }                   
    ElseIf ($choice -eq '5') {$NumberOf = Read-Host -Prompt "Enter number of times to pull CPU Load: Recomend (3 or 5)" 
    ...
    ElseIf ($choice -eq '7') {gwmi –ComputerName $serverName win32_logicaldisk | Where DeviceID -ne 'A:' | Where DeviceID -ne 'D:' | Format-Table -AutoSize DeviceID,
...
$yesNo = Read-Host
    If ($yesNo -eq "y") {C:\AutoServices.ps1}
